...
(function(window) {
	...
	window.fetcher = function(input, init) {
		...
		xhr.onload = function() {
			...
			if (options.status === 200) {
				deferred.resolve(new Response(body, options));
			} else {
				deferred.reject({
					status: options.status || 500,
					message: options.statusText || 'Internal Service Error',
				});
			}
		};
		...
		deferredPromise.reject = deferred.reject;
		deferredPromise.resolve = deferred.resolve;
